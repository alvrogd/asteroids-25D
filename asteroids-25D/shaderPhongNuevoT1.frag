...
void main()
{
	...
	// material
	vec3 ambiental = puntoLuz.ambiental * vec3(texture(material.texturaDifusa, TexCoord));
	...
	vec3 difusa = puntoLuz.difusa * diferencia * vec3(texture(material.texturaDifusa, TexCoord));
	...
	float spec = pow(max(dot(direccionCamara, direccionReflexion), 0.0), material.brillo);

	// Ahora se puede calcular ya el resultado de la componente especular
	vec3 especular = puntoLuz.especular * spec * vec3(texture(material.texturaEspecular, TexCoord));
	...
	FragColor = texture(Textura1, TexCoord) * vec4(resultado, 1.0f);
